...
logins as (
    select  
        ...
        case when total_logins >= 1 then max(emails.instance_account_id) else null end as login_flag,
        date(max(emails.agent_last_login_timestamp)) as last_login_date
    from cart_visits cart_visits_
    left join propagated_foundational.product_agent_info.dim_agent_emails_daily_snapshot emails
        on 
            emails.instance_account_id = cart_visits_.account_id
            --- Logins from last 30 days before win date
            and date_trunc('day', emails.agent_last_login_timestamp) >= dateadd('day', -30, cart_visits_.first_cart_visit)
            --and date_trunc('month', emails.agent_last_login_timestamp) = wins_.month_win
            and date_trunc('day', emails.agent_last_login_timestamp) <= cart_visits_.first_cart_visit
            and (emails.agent_role in ('Admin', 'Billing Admin') or emails.agent_is_owner = True)
    group by all
)
