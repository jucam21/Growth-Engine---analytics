...
with accounts as (
    select
        ...
        -- Min number total tickets 
        case
            when
                cast({{TICKETS_RULE_ALL}} as string) = 'all'
                then 1
            when
                support_ticket_counts['total_tickets'] >= {{TICKETS_RULE_ALL}}
                then 1
            else 0
        end as tickets_filter_all,
...
accounts_funnel as (
    select 
        ...
        TRIAL_AGE,
        TRIAL_SKU_NAMES,
        support_ticket_counts['total_tickets'] as total_tickets,
