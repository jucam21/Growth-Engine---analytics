...
select *
from follow_up_subscription




with max_date_finance as (
    select
...
select 
    count(*) as total_count,
    count(distinct account_id) as total_count_distinct,
    sum(renewal_passed) as total_renewal_passed,
    count(distinct case when renewal_passed = 1 then account_id else null end) as unique_renewal_passed
from passed
...
where zendesk_account_id = 22169213
...
on finance.billing_account_id = snapshot.billing_account_id
    and finance.service_date = snapshot.source_snapshot_date
where 
    snapshot.instance_account_id = 21177553
    and finance.service_date >= '2025-04-01'
    and (
            finance.subscription_kind = 'Primary'
            or finance.subscription_kind is null
        )
order by service_date, billing_account_id



-- Check coupon period term
...
with redeemed_zuora as (
    ...
    from cleansed.zuora.zuora_rate_plan_charges_bcv as zuora
        ...
        left join cleansed.zuora.zuora_rate_plan_charge_tiers_bcv as tiers
            on zuora.id = tiers.rate_plan_charge_id
        inner join
            (select distinct account_id from sandbox.juan_salgado.ge_dashboard_test) accounts
            on mapping.instance_account_id = accounts.account_id
    where
        zuora.is_last_segment = true
        and subscription.status in ('Active', 'Expired')
        and zuora.created_date >= '2025-03-01'
        and LOWER(coupon_id) like '%save%'
    group by
        all
)
